    ...
    const handleGenerateFromOrders = async () => {
        ...
        try {
            const res = await fetch('/api/production/runs', {
                ...
                body: JSON.stringify({
                    planFromOrders: true,
                    run_date: new Date().toISOString().split('T')[0] // Default to today
                })
            });

            if (res.ok) {
                const newRun = await res.json();
                toast.success(`Generated run with ${newRun._count?.tasks || 0} tasks!`);
                setRuns([newRun, ...runs]);
            } else {
                const data = await res.json();
                toast.error(data.error || "No pending orders to process.");
            }
        } catch (e) {
            toast.error("Failed to generate production run.");
        } finally {
            setIsLoading(false);
        }
    };
            ...
            <div className="flex justify-between items-center mb-6">
                ...
                <div className="flex gap-3">
                    <button
                        ...
                        disabled={isLoading}
                        className="flex items-center gap-2 bg-emerald-600 text-white px-5 py-2.5 rounded-xl font-bold shadow-lg shadow-emerald-500/20 hover:bg-emerald-700 transition-all active:scale-95 disabled:opacity-50"
                    >
