...
export default function DeliveryDashboard() {
    ...
    // Week filter state: null = "All Weeks"
    const [selectedWeekStart, setSelectedWeekStart] = useState<Date | null>(() => {
        if (typeof window !== 'undefined') {
            const saved = localStorage.getItem('delivery_week_start');
    ...
    useEffect(() => {
        if (selectedWeekStart) {
            localStorage.setItem('delivery_week_start', toDateString(selectedWeekStart));
        } else {
            localStorage.setItem('delivery_week_start', 'all');
        }
    }, [selectedWeekStart]);

    const weekParam = selectedWeekStart ? `&delivery_week_start=${toDateString(selectedWeekStart)}` : '';
    const weekSearchParam = selectedWeekStart ? `?delivery_week_start=${toDateString(selectedWeekStart)}` : '';
    ...
    return (
        <div className="max-w-7xl mx-auto p-4 md:p-6 pb-24">
            ...
            {/* Week Selector */}
            <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 mb-6 flex items-center justify-between shadow-sm">
                <div className="flex items-center gap-3">
                    <Calendar size={18} className="text-indigo-500" />
                    <span className="text-sm font-bold text-slate-700 dark:text-slate-200">Delivery Week</span>
                </div>
                <div className="flex items-center gap-2">
                    <button
                    ...
                    <div className="flex items-center gap-1 bg-slate-50 dark:bg-slate-900 rounded-lg px-1 py-0.5">
                        <button
                            onClick={() => {
                                const prev = new Date(selectedWeekStart || getISOMonday(new Date()));
                        ...
                        <button
                            onClick={() => setSelectedWeekStart(getISOMonday(new Date()))}
                            className={`px-3 py-1 rounded-lg text-xs font-bold transition-colors min-w-[140px] text-center ${
                                selectedWeekStart && toDateString(selectedWeekStart) === toDateString(getISOMonday(new Date()))
                                    ? 'bg-indigo-600 text-white'
                                    : selectedWeekStart
                                        ? 'bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 shadow-sm border border-slate-200 dark:border-slate-600'
                                        : 'bg-slate-100 dark:bg-slate-700 text-slate-500 hover:bg-slate-200 dark:hover:bg-slate-600'
                            }`}
                        >
                            {selectedWeekStart ? formatWeekRange(selectedWeekStart) : 'This Week'}
                        </button>
                        <button
                            onClick={() => {
                                const next = new Date(selectedWeekStart || getISOMonday(new Date()));
