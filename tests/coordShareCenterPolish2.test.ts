...
describe('COORD-SHARE-CENTER-POLISH-2', () => {
    ...
    describe('16. scope contains no backend/schema/migration files', () => {
        it('the working-tree diff touches only ShareCenter.tsx and test files', () => {
            ...
            // CRM-LEAD-D1 — a later, separately-authorized phase: fundraiser supporters stopped
            // being counted and listed as sales leads in the Customer CRM. Classification only —
            // one predicate, the flag the customers route resolves from it, and the two surfaces
            // that read it. No schema, no migration, no persisted status, no money.
            const CRM_LEAD_D1 = [
                'lib/fundraiserLead.ts',
            ...
            // CRM-CAMPAIGN-DETAILS-1 — a later, separately-authorized phase: the tenant
            // can correct a live fundraiser's delivery date/time, supporter deadline,
            // pickup location and bundle goal, and the legacy organization-profile sync
            // can no longer overwrite those corrections. No schema, no migration, no
            // financial field.
