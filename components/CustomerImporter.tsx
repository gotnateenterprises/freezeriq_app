...
import { Upload, Users, CheckCircle, AlertCircle, X, RefreshCw, FileText } from 'lucide-react';
import { useRouter } from 'next/navigation';
import { toast } from 'sonner';

export default function CustomerImporter({ onClose }: { onClose: () => void }) {
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl max-w-2xl w-full flex flex-col max-h-[90vh] overflow-hidden border border-slate-200 dark:border-slate-700">
                ...
                <div className="flex justify-between items-center p-6 border-b border-slate-100 dark:border-slate-800">
                    <div>
                        ...
                        <p className="text-slate-500 dark:text-slate-400 mt-1 text-sm">Upload a CSV export from your Square Customer Directory.</p>
                    </div>
                    ...
                    {status === 'idle' || status === 'error' ? (
                        <div className="space-y-6">
                            <div
                                ...
                                className={`border-2 border-dashed rounded-2xl p-12 flex flex-col items-center justify-center cursor-pointer transition-all ${file ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-900/10' : 'border-slate-300 dark:border-slate-700 hover:border-indigo-400 hover:bg-slate-50 dark:hover:bg-slate-800/50'}`}
                            >
                                <input
                                    type="file"
                                    ref={fileInputRef}
                                ...
                                {file ? (
                                    <>
                                        <FileText size={48} className="text-indigo-600 dark:text-indigo-400 mb-4" />
                                    ...
                                    <>
                                        ...
                                        <p className="text-slate-400 text-sm mt-1">Exported from Square Dashboard</p>
                                    </>
                                )}
                            </div>
                            ...
                            <div className="bg-amber-50 dark:bg-amber-900/10 border border-amber-100 dark:border-amber-900/30 p-4 rounded-xl text-xs text-amber-700 dark:text-amber-400">
                                <strong>Tip:</strong> Ensure your CSV includes columns for First Name, Last Name, and Email Address. The system will automatically map these for you.
                            </div>
                            ...
                            <div className="flex justify-end pt-4">
                                <button
                                    ...
                                    className="px-8 py-3 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-xl font-bold transition-all shadow-lg shadow-indigo-200 dark:shadow-none flex items-center gap-2"
                                >
                                    {isUploading ? (
                                        <>
                        ...
                        <div className="space-y-6 text-center py-8">
                            ...
                            <p className="text-slate-500 dark:text-slate-400 italic">Customers have been merged and updated.</p>
