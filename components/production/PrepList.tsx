...
export default function PrepList({ items, onRefresh }: PrepListProps) {
    ...
    const handleUpdateStatus = async (bundleId: string, currentStatus: string, newStatus: string, customerId?: string) => {
        ...
        try {
            const res = await fetch('/api/orders/batch-update-by-bundle', {
                ...
                body: JSON.stringify({ bundleId, currentStatus, newStatus, customerId })
            });
        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4">
            ...
            {approvedItems.length > 0 && (
                <div className="space-y-6">
                    ...
                    {Object.entries(
                        approvedItems.reduce((acc: Record<string, PrepItem[]>, item: PrepItem) => {
                        ...
                        <div key={customerName} className="bg-slate-50/50 dark:bg-slate-900/20 p-6 rounded-3xl border border-slate-100 dark:border-slate-700/50 space-y-4">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 print:grid-cols-2 print:gap-4">
                                {(itemsList as PrepItem[]).map((item: PrepItem) => (
                                    <div key={`${item.customer_id}-${item.bundle_id}`} className="bg-white dark:bg-slate-800 p-6 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm relative overflow-hidden group hover:border-amber-200 dark:hover:border-amber-900/50 transition-colors print:break-inside-avoid print:border-slate-300">
                                        ...
                                        <div className="relative z-10">
                                            ...
                                            <button
                                                onClick={() => handleUpdateStatus(item.bundle_id, 'APPROVED', 'IN_PRODUCTION', item.customer_id)}
