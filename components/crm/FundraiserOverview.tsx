    ...
    const handleSendEmail = async (subject: string, html: string, attachments: any[] = []) => {
        try {
            ...
            if (res.ok) {
                ...
                // the customer forward for a message nobody received, and the
                // tenant then worked a pipeline that had run ahead of reality.
                //
                            ...
                            <div className="space-y-6">
                                <div className="bg-emerald-50 dark:bg-emerald-900/20 p-6 rounded-2xl border border-emerald-100 dark:border-emerald-800 flex items-center justify-between">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        onClick={async () => {
                                            ...
                                            if (!readiness.ready) {
                                                alert(launchReadinessMessage(readiness.missing));
                                                return;
                                            }
                                ...
                                <FundraiserSetup
                                    ...
                                    onSave={async (data) => {
                                        ...
                                        // Launch Readiness Gate (re-check with freshly-saved data)
                                        const freshCustomer = { ...customer, fundraiser_info: data };
                                        const readiness = validateLaunchReadiness(freshCustomer);
                                        if (!readiness.ready) {
                                            alert(launchReadinessMessage(readiness.missing));
                                            return;
                                        }
