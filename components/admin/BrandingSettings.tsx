...
export default function BrandingSettings({ isSuperAdmin }: BrandingSettingsProps) {
    ...
    useEffect(() => {
        ...
        // Load QRs from localStorage (as temporary cache)
    ...
    const handleBrandingSave = async () => {
        ...
        try {
            ...
            // Handle Review QR Code
            const reviewQr = qrCodes.find(q => q.name.toLowerCase().includes('review')) || qrCodes[0];
            if (reviewQr && reviewQr.data.startsWith('data:')) {
                const res = await fetch(reviewQr.data);
                const blob = await res.blob();
                formData.append('review_qr', blob, 'review-qr.png');
            }
    ...
    const generateQrCode = async () => {
        ...
        try {
            ...
            reader.onloadend = () => {
                ...
                localStorage.setItem('qrCodes', JSON.stringify([...qrCodes, newQr]));
                setQrInput("");
            };
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 p-8 space-y-8">
            ...
            <div className="pt-8 border-t border-slate-100 dark:border-slate-700">
                <div className="flex items-center justify-between mb-4">
                    <label className="block text-sm font-bold text-slate-700 dark:text-slate-300">Review QR Codes</label>
                    <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Customer Feedback</span>
                </div>
                <div className="flex gap-2 mb-6">
                    <div className="relative flex-1">
                        ...
                        <input
                            ...
                            className="w-full pl-10 pr-4 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg text-sm outline-none focus:ring-2 focus:ring-indigo-500 font-mono"
                            placeholder="Paste Review Link (Google/Yelp)..."
                        />
                    </div>
                    <button
                        onClick={generateQrCode}
                        disabled={isGenerating || !qrInput}
                        className="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2 rounded-lg text-sm font-bold shadow-sm transition-all active:scale-95 disabled:opacity-50 flex items-center gap-2"
                    >
                        {isGenerating ? 'Generating...' : <><Plus size={16} /> New QR</>}
                    </button>
                </div>

                <div className="flex gap-4 overflow-x-auto pb-4 custom-scrollbar">
                    {qrCodes.map((qr) => (
                        <div key={qr.id} className="relative group shrink-0 w-20 h-20 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-1 shadow-sm">
                            <img src={qr.data} alt="QR" className="w-full h-full object-contain" />
                            <div className="absolute inset-0 bg-rose-600/90 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center rounded-xl">
                                <button onClick={() => deleteQr(qr.id)} className="text-white p-2">
                                    <Trash2 size={16} />
                                </button>
                            </div>
                        </div>
                    ...
                    {qrCodes.length === 0 && (
                        <div className="flex items-center justify-center w-full py-8 border-2 border-dashed border-slate-100 dark:border-slate-800 rounded-2xl text-slate-400 text-sm italic">
