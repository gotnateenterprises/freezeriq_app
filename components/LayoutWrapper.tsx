...
export default function LayoutWrapper({ children, hasSession }: LayoutWrapperProps) {
    ...
    return (
        <div className="flex h-full print:block print:h-auto w-full">
            ...
            <main className={`flex-1 ${!isShopPage ? 'h-full overflow-y-auto p-8' : 'min-h-screen p-4 sm:p-8'} overflow-x-clip ${showSidebar ? 'ml-[280px]' : 'ml-0'} print:ml-0 print:p-0 print:h-auto print:overflow-visible transition-all duration-300`}>
                {children}
            </main>
        </div>
    );
